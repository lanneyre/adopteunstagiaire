...
class Stagiaire extends Utilisateur
{
	...
	public static function getNbStagiaire($formation_id = 1){
	...
	public static function getAllStagiaire($formation_id = 1, $offset = 0, $nbCard = 20){
		// je récupère ici tous les apprenants de la promotion
        // la requete
        $sqlAllApprenants = "SELECT * FROM `stagiaire` AS s JOIN `utilisateur` AS u ON (s.`stagiaire_utilisateur_id` = u.`utilisateur_id`) WHERE s.`stagiaire_formation_id` = :stagiaire_formation_id ORDER BY s.`stagiaire_prenom` ASC LIMIT :offset, :nbCard;";
        // que j'envoie au serveur
        $requeteAllApprenants = $GLOBALS['db']->prepare($sqlAllApprenants);
        // j'attache l'id de la formation à ma requete
        $requeteAllApprenants->bindValue(":stagiaire_formation_id", $formation_id);
